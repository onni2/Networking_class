...
int main(int argc, char* argv[]) {
    ...
    while(true) {
        ...
        // Connect to server for each command
        ...
        if(tokens[0] == "SENDMSG" && tokens.size() >= 3) {
            ...
            // Client sends just: GETMSG (no parameters needed)
            ...
            if(sendCommand(serverSocket, cmd)) {
                ...
                if(receiveCommand(serverSocket, response)) {
                    ...
                    logMessage("ERROR: Failed to receive response");
                }
            } else {
            ...
            // Client sends: LISTSERVERS (not STATUSREQ!)
